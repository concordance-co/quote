...
def _maybe_register_fullpass_debug_routes(app: FastAPI) -> None:
    # Fullpass debug is heavy (HF backend + activation storage) and is not needed for
    # normal OpenAI-compatible serving. Keep it opt-in to avoid dragging extra deps
    # into deployed environments.
    raw = os.environ.get("CONCORDANCE_ENABLE_FULLPASS_DEBUG", "")
...
def create_app(model_id: str | None = None, *, remote: bool = False) -> FastAPI:
    ...
    _maybe_register_fullpass_debug_routes(app)
    logger.warning(
    ...
    async def analyze_features(body: dict = Body(...)):
        ...
        Body format:
        {
            "feature_timeline": [...],  # From extract_features
            "injection_positions": [5, 10],  # Optional
            "context": "User injected 'test' at position 5"  # Optional context
        }
        """
        import httpx
        import anthropic
        ...
        if not timeline or not isinstance(timeline, list):
            raise HTTPException(status_code=400, detail="feature_timeline is required")
        ...
        for entry in timeline:
            for feat in entry.get("top_features", [])[:5]:  # Top 5 per position
                fid = feat.get("id")
            ...
            try:
                async with httpx.AsyncClient(timeout=10.0) as client:
                    ...
                    if resp.status_code == 200:
                        data = resp.json()
                        # Get the auto-generated explanation
                ...
                logger.warning(f"Failed to fetch Neuronpedia description for {feature_id}: {e}")
        ...
        for fid, activation in top_features:
            ...
            feature_descriptions[fid] = {"activation": activation, "description": desc}
        ...
        for entry in timeline:
            ...
            timeline_summary.append(f"Pos {pos}: '{token}'{marker} -> {', '.join(feat_strs)}")
...
{chr(10).join(timeline_summary[:50])}  # Limit to 50 positions
...
Focus on insights that would help someone understand what the model is "thinking" during this generation."""
        ...
        return {
            "analysis": analysis,
            "top_features": [
                {"id": fid, "activation": info["activation"], "description": info["description"]}
                for fid, info in feature_descriptions.items()
            ]
        }
    ...
    async def extract_features(body: dict = Body(...)):
        ...
        Body format:
        {
            "tokens": [1, 2, 3, ...],  # List of token IDs
            "top_k": 20,               # Optional: number of top features per position
            "layer": 16,               # Optional: which layer to extract from
            "injection_positions": [5, 10]  # Optional: positions to compare before/after
        }
            ...
            "feature_timeline": [
                {
                    ...
                    "token": 123,
                    "token_str": "Hello",
                    "top_features": [{"id": 456, "activation": 0.95}, ...]
                },
                ...
            ],
            "comparisons": [...]  # Only if injection_positions provided
        }
        """
        if not isinstance(body, dict):
            raise HTTPException(status_code=400, detail="body must be a JSON object")
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail="tokens must be a list of integers"
            )
        ...
        try:
            ...
            # Determine model ID for feature extraction based on loaded model
            # For Llama models, we use the HF model ID
            # Using 8x SAE (32K features) because it's on Neuronpedia
            ...
            # If injection positions provided, add comparisons
            if injection_positions and isinstance(injection_positions, list):
                try:
                    ...
                    # Invalid injection positions, skip comparisons
                    pass
