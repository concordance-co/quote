...
export function LogShareDialog({
  open,
  onOpenChange,
  logData,
  onStatusChange,
  ...
  const { timeline } = useTokenTimeline(logData);
  ...
  useEffect(() => {
    setLocalToken(logData.public_token);
  }, [logData.public_token]);

  const effectiveToken = localToken || logData.public_token;
  const effectiveIsPublic = logData.is_public || !!localToken;
  ...
  const handleShareTwitter = useCallback(() => {
    if (!shareUrl) return;
    const tweetText = "Check out this inference log on Concordance!";
  ...
  const handleMakePublic = useCallback(async () => {
    try {
      setUpdating(true);
      setError(null);
      const response = await makeRequestPublic(logData.request_id);
  ...
  }, [logData.request_id, onStatusChange]);

  const handleMakePrivate = useCallback(async () => {
    try {
      setUpdating(true);
      setError(null);
      await makeRequestPrivate(logData.request_id);
      setLocalToken(null);
  ...
  }, [logData.request_id, onStatusChange, onOpenChange]);
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-md">
          ...
          <DialogFooter>
            ...
            <Button onClick={handleMakePublic} disabled={updating}>
              {updating ? (
                <>
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-center bg-muted/30 rounded-lg p-3 border border-border">
              ...
              >
                ...
                >
                  <ShareableCard
                    ref={cardRef}
                    model={logData.model_id || "unknown"}
                    maxTokens={logData.max_steps || 256}
                    systemPrompt={logData.system_prompt || ""}
                    userPrompt={logData.user_prompt || ""}
                    outputText={logData.final_text || ""}
                    shareUrl={shareUrl || ""}
                    tokens={tokens}
                    showInjection={false}
                  />
            ...
            <div className="flex items-center justify-between text-[11px] text-muted-foreground">
              <span>Anyone with the link can view this log.</span>
              <Button
                variant="ghost"
                size="sm"
      ...
      <Dialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Make Log Private?</DialogTitle>
            <DialogDescription>
              This will revoke the public link. Anyone with the current link
              will no longer be able to view this log.
            </DialogDescription>
          </DialogHeader>
          {error && (
            <div className="text-sm text-destructive bg-destructive/10 p-3 rounded">
